       ...
       public void OnEnable()
        ...
        internal void MovementSystem(float DeltaTime)
        {
            ...
            SpeedMultiplier = (LerpAnimator > 0) ?
                Mathf.MoveTowards(SpeedMultiplier, CurrentSpeedModifier.animator.Value, LerpAnimator) :
                ...
                if (AnimatePhysics && RB)
                {
                    ...
                    else
                    {
                        ...
                        if (DeltaTime > 0)
                        {
                            ...
                            if (UseGravity && GoingDown)
                            {
                                var RBOldDown = Vector3.Project(OldRBVelocity, Gravity);
                                var RBNewDown = Vector3.Project(NewRBVelocity, Gravity);
                                var NewDMagn = RBNewDown.sqrMagnitude;
                                var Old_DMagn = RBOldDown.sqrMagnitude;

                                HACKCOUNT++;

                                if (NewDMagn > (Old_DMagn * 2) && HACKCOUNT > 5) //5 seems to be good
                                {
                                    //Debug.Log($"Reset RB");
                                    ResetUPVector();
        ...
        public void CalculateTargetSpeed()
        {
            ...
            //Calculate these Once per Cycle Extremely important 
            TargetSpeed = TargetDir * Speed_Modifier * DeltaTime;              
            HorizontalSpeed = Vector3.ProjectOnPlane(Inertia, UpVector).magnitude;  
        }
